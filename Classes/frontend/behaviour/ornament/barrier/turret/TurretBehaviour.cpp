...
TurretBehaviour::TurretBehaviour(GeographicNode* geographicNode) {
    ...
    this->property = new OrnamentProperty("turret", id, "attack", 2, 3.5f, 2);
    this->stateMachine = new FiniteStateMachine<TurretBehaviour>(this);
    this->stateMachine->add("destroy", new TurretDestroyState());
    this->stateMachine->add("damage", new TurretDamageState());
    this->stateMachine->add("touchBegan", new TurretTouchBeganState());
