...
GoldStrageTouchEndState::GoldStrageTouchEndState() {}

GoldStrageTouchEndState::~GoldStrageTouchEndState() {}

void GoldStrageTouchEndState::create(Parameter* parameter) {
    ...
    for (std::vector<GeographicNode*>::iterator it = nodeVector.begin(); it != nodeVector.end(); it++) {
        ...
        if (areaNode->geographicId == node->geographicId) {
            areaNode->type = BarrierBehaviourType::GoldStrage;
        } else {
            areaNode->type = BarrierBehaviourType::GoldStrage + BarrierBehaviourType::AroundArea;
        }
