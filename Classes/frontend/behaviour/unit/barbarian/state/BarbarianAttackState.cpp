...
BarbarianAttackState::BarbarianAttackState() { this->damageStrategy = ServiceGateway::getInstance()->request("service://barrier/damage"); }
BarbarianAttackState::~BarbarianAttackState() {}
void BarbarianAttackState::create(Parameter* parameter) {
    this->destroy = false;
...
void BarbarianAttackState::changeDirection() {
...
void BarbarianAttackState::selectAttackAnimation(std::string animationName, bool transform) {
    if (false != this->destroy) {
        return;
    }
    Parameter parameter;
